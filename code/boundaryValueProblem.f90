! File: boundaryValueProblem.f90
! Purpose: Solve periodic shifted Poisson problem with spectral method

program boundaryValueProblem
    use utility, only: fp, pi
    !! !! === 1. import the required data, subroutines, and functions
    !! !!       from problemsetup.f90 and discreteFourierTransform.f90 here.
    use discreteFourierTransform, only: matvecprod, discreteFourierTransform_InvTransMat, discreteFourierTransform_TransMat
    use problemsetup, only: exact_sol, forcing, problemsetup_Init, N, outFile
    implicit none
    ...
    f = forcing(x)
    !! !! === 4. Fill in k and T
    call discreteFourierTransform_TransMat(x,k,T)
    !! !! === 5. Fill in Tinv
    call discreteFourierTransform_InvTransMat(x,k,Tinv)
    !! !! === 6. Set ft = T*f using matvecprod
    ft = matvecprod(T,f)
    !! !! === 7. Apply wavenumber scaling
    ft = ft / (1 + k**2) !columns of [a1,b1, a2,b2, a3,b3,..., an,bn]
...
contains
    ...
    subroutine write_data()
        ...
        do i = 1, N
            write (20, *) x(i), u(i), f(i), ft(i), exact_sol(x(i))
        end do
        close (20)
    end subroutine write_data

end program boundaryValueProblem
